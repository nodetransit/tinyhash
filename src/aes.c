...
void ccm_prepare_first_ctr_blk(unsigned char counter[], const unsigned char nonce[], unsigned int nonce_len, unsigned int payload_len_store_size);
void ccm_prepare_first_format_blk(unsigned char buf[], unsigned int assoc_len, unsigned int payload_len, unsigned int payload_len_store_size, unsigned int mac_len, const unsigned char nonce[], unsigned int nonce_len);
void ccm_format_assoc_data(unsigned char buf[], unsigned int* end_of_buf, const unsigned char assoc[], unsigned int assoc_len);
void ccm_format_payload_data(unsigned char buf[], unsigned int* end_of_buf, const unsigned char payload[], unsigned int payload_len);
...
rotate_lsb_to_end(unsigned int x)
{
    return (((x) << 8) | ((x) >> 24));
}
...
void
xor_buf(const unsigned char in[],
        unsigned char out[],
        size_t len)
{
    size_t idx;
...
int
aes_encrypt_cbc_mac(const unsigned char in[],
                    size_t in_len,
                    unsigned char out[],
                    const unsigned int key[],
                    int keysize,
                    const unsigned char iv[])
{
    ...
    unsigned char buf_out[AES_BLOCK_SIZE];
...
void
aes_encrypt_ctr(const unsigned char in[],
                size_t in_len,
                unsigned char out[],
                const unsigned int key[],
                int keysize,
...
void
aes_decrypt_ctr(const unsigned char in[],
                size_t in_len,
                unsigned char out[],
                const unsigned int key[],
                int keysize,
 ...
 * @param nonce_len
 * @param out
 * @param out_len payload_len + assoc_len
 * @param mac_len
 * @param key_str
 * @param keysize
 * @return
 */
int
aes_encrypt_ccm(const unsigned char payload[],
                ...
                int keysize)
{
    ...
    unsigned char counter[AES_BLOCK_SIZE], mac[16], * buf;

    if (mac_len != 4 &&
        mac_len != 6 &&
        mac_len != 8 &&
        mac_len != 10 && mac_len != 12
        && mac_len != 14 &&
        mac_len != 16) {
        return 0;
    }
    ...
    buf = (unsigned char*)malloc(payload_len + assoc_len + 48 /*Round both payload and associated data up a block size and add an extra block.*/);

    if (!buf) {
        return 0;
    }

    unsigned int end_of_buf;
    unsigned int payload_len_store_size;
    unsigned int key[60];
...
int
aes_decrypt_ccm(const unsigned char ciphertext[],
                ...
                int keysize)
{
    ...
    unsigned char mac_buf[16];
    unsigned char* buf;
    unsigned int end_of_buf;
    unsigned int plaintext_len_store_size;
    unsigned int key[60];
    ...
    buf = (unsigned char*)malloc(assoc_len + ciphertext_len /*ciphertext_len = plaintext_len + mac_len*/ + 48);
...
void
ccm_prepare_first_ctr_blk(unsigned char counter[],
                          const unsigned char nonce[],
...
void
ccm_prepare_first_format_blk(unsigned char buf[],
...
void
ccm_format_assoc_data(unsigned char buf[],
                      unsigned int* end_of_buf,
                      const unsigned char assoc[],
                      unsigned int assoc_len)
{
    ...
    *end_of_buf += assoc_len;
    unsigned int pad = AES_BLOCK_SIZE - (*end_of_buf % AES_BLOCK_SIZE); /*BUG?*/
...
void
ccm_format_payload_data(unsigned char buf[],
                        unsigned int* end_of_buf,
                        ...
                        unsigned int payload_len)
{
    ...
    *end_of_buf += payload_len;
    ...
    unsigned int pad = *end_of_buf % AES_BLOCK_SIZE;
    if (pad != 0) {
...
void
aes_key_setup(const unsigned char key[],
              unsigned int w[],
              int keysize)
{
    int Nb                    = 4;
    ...
    for (idx = Nk; idx < Nb * (Nr + 1); ++idx) {
        ...
        if ((idx % Nk) == 0) {
            temp = SubWord(rotate_lsb_to_end(temp)) ^ Rcon[(idx - 1) / Nk];
...
void
AddRoundKey(unsigned char state[][4],
...
void
SubBytes(unsigned char state[][4])
{
...
void
InvSubBytes(unsigned char state[][4])
{
...
void
ShiftRows(unsigned char state[][4])
{
    int t;
...
void
InvShiftRows(unsigned char state[][4])
{
    int t;
...
void
MixColumns(unsigned char state[][4])
{
    unsigned char col[4];
...
void
InvMixColumns(unsigned char state[][4])
{
    unsigned char col[4];
...
void
aes_encrypt(const unsigned char in[], unsigned char out[], const unsigned int key[], int keysize)
{
...
void
aes_decrypt(const unsigned char in[], unsigned char out[], const unsigned int key[], int keysize)
{
    unsigned char state[4][4];
...
void print_state(unsigned char state[][4])
{
    int idx,idx2;
